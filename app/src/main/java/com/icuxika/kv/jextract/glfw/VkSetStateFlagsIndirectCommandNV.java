// Generated by jextract

package com.icuxika.kv.jextract.glfw;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct VkSetStateFlagsIndirectCommandNV {
 *     uint32_t data;
 * };
 * }
 */
public class VkSetStateFlagsIndirectCommandNV {

    public static MemoryLayout $LAYOUT() {
        return constants$828.const$5;
    }
    public static VarHandle data$VH() {
        return constants$829.const$0;
    }
    /**
     * Getter for field:
     * {@snippet :
     * uint32_t data;
     * }
     */
    public static int data$get(MemorySegment seg) {
        return (int)constants$829.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * uint32_t data;
     * }
     */
    public static void data$set(MemorySegment seg, int x) {
        constants$829.const$0.set(seg, x);
    }
    public static int data$get(MemorySegment seg, long index) {
        return (int)constants$829.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void data$set(MemorySegment seg, long index, int x) {
        constants$829.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


